...
function setSessionCookie(c, sid) {
...
function renderLogin(error) {
    return layout("Login", `<div class="card" style="max-width: 420px; margin: 0 auto;">
      <h1>Admin Login</h1>
...
function renderStateCard(state, error) {
    if (error) {
    ...
    for (const c of carts) {
        if (!cartsByUser.has(c.user_id))
            cartsByUser.set(c.user_id, []);
    ...
    const sessionIndicator = (uid) => {
        const rows = sessionsByUser.get(uid) ?? [];
        if (rows.length === 0)
            return `<span class="session-dot none" title="no sessions">—</span>`;
    ...
    const cartsHtml = users
        ...
        const rows = items
            .map((i) => `<li>${i.quantity}× ${escapeHtml(i.product)} <span class="muted">@ ${fmtPrice(i.price)}</span></li>`)
            .join("");
...
async function fetchState() {
    try {
        ...
        if (!res.ok)
            throw new Error(`state fetch ${res.status}`);
...
async function fetchModeData() {
    ...
    try {
        ...
        if (!modeRes.ok)
            throw new Error(`mode fetch ${modeRes.status}`);
        if (!modesRes.ok)
            throw new Error(`modes fetch ${modesRes.status}`);
        const currentMode = (await modeRes.json()).mode;
...
async function renderPanel(flash) {
    const [{ currentMode, modes, error: modeError }, { state, error: stateError }] = await Promise.all([fetchModeData(), fetchState()]);
    const groups = groupModes(modes);
    const groupsHtml = groups
        .map((g) => `<div class="section-group">
    ...
    return layout("Panel", `${errorHtml}${flashHtml}
    <div class="card">
      <div class="card-header">
        <h1>Bug Mode</h1>
      </div>
...
routes.post("/panel/mode", async (c) => {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            return c.html(await renderPanel({ kind: "error", msg: `Main app returned ${res.status}: ${text}` }), 502);
        }
    }
    catch (e) {
        return c.html(await renderPanel({ kind: "error", msg: `Request failed: ${e?.message ?? String(e)}` }), 502);
    }
